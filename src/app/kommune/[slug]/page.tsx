...
export default async function KommuneSide({ params }: Props) {
  ...
  const kommuneFAQ = [
    { sporsmal: `Hva koster elektriker i ${kommune.navn}?`, svar: `Timeprisen for elektriker i ${kommune.navn} ligger typisk mellom 750 og 1 200 kr inkludert moms. Vi anbefaler å innhente minst tre tilbud for å sikre best pris.` },
    { sporsmal: `Finnes det autoriserte elektrikere i ${kommune.navn}?`, svar: `Ja, via Elspesialisten kan du komme i kontakt med DSB godkjente og autoriserte elektrikere i ${kommune.navn}. Alle våre partnere er lisensiert og arbeider etter gjeldende forskrifter.` },
    { sporsmal: `Kan jeg få hjelp av akutt elektriker i ${kommune.navn}?`, svar: `Ja, vi formidler akutt elektriker i ${kommune.navn}. Meld din forespørsel med hastegrad «Akutt» og oppgi at det haster.` },
  ];
  ...
  return (<><Header /><main id="main-content">
    ...
    <section className="section-gradient hero-pattern" aria-labelledby="kommune-hero-heading">
      <div className="container-site py-12 sm:py-16"><div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
        <div>
          <div className="badge-primary mb-4"><MapPin className="w-3 h-3" aria-hidden />{kommune.fylke}</div>
          <h1 id="kommune-hero-heading" className="font-display font-extrabold text-display-xl text-secondary-950 mb-4 text-balance">Elektriker i <span className="text-gradient-primary">{kommune.navn}</span></h1>
          <p className="text-body-lg text-secondary-600 mb-6 text-pretty">{kommune.kortTekst}</p>
          <div className="flex flex-wrap gap-3 mb-6">
            {[{ ikkon: Shield, tekst: "DSB autoriserte elektrikere" }, { ikkon: Clock, tekst: "Svar innen 24 timer" }, { ikkon: Star, tekst: "Gratis tilbud" }].map(({ ikkon: Icon, tekst }) => (<div key={tekst} className="flex items-center gap-2 badge-neutral"><Icon className="w-3.5 h-3.5" aria-hidden />{tekst}</div>))}
          </div>
    ...
    <section className="section-white section-py-sm"><div className="container-site max-w-prose">
      ...
      <div className="text-body-md text-secondary-600 leading-relaxed space-y-4"><p>{kommune.langTekst}</p></div>
    </div></section>
