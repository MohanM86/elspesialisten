...
import { BERGEN_BYDELER, getBergenBydel, getAllBergenBydelSlugs } from "@/data/bergen-bydeler";
import { buildFAQSchema } from "@/lib/utils";

interface Props { params: Promise<{ bydel: string }>; }

export async function generateStaticParams() {
  return getAllBergenBydelSlugs().map((bydel) => ({ bydel }));
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const bydel = getBergenBydel(slug);
  if (!bydel) return {};
  const bedrifter = BERGEN_BEDRIFTER.filter((b) => inRange(b.p, bydel.postRanges));
  return {
    title: `Elektriker ${bydel.navn}, Bergen | ${bedrifter.length} bedrifter | Elspesialisten`,
    description: `Finn elektriker i ${bydel.navn} i Bergen. ${bedrifter.length} registrerte bedrifter i bydelen. Sikringsskap, elbillader, feilsøking og akutt hjelp.`,
    alternates: { canonical: `https://elspesialisten.no/kommune/bergen/${slug}` },
  };
}

export default async function BergenBydelSide({ params }: Props) {
  const { bydel: slug } = await params;
  const bydel = getBergenBydel(slug);
  if (!bydel) notFound();

  const bedrifter = BERGEN_BEDRIFTER.filter((b) => inRange(b.p, bydel.postRanges));
  const andreBydeler = BERGEN_BYDELER.filter((b) => b.slug !== slug);

  const faqItems = [
    { sporsmal: `Hva koster elektriker i ${bydel.navn}?`, svar: `Timeprisen for elektriker i ${bydel.navn} er den samme som i Bergen generelt: mellom 800 og 1 300 kr inkludert moms. Prisen varierer etter oppdragstype og tidspunkt.` },
    ...
    { sporsmal: `Trenger jeg oppgradere sikringsskapet i ${bydel.navn}?`, svar: `${bydel.beskrivelse} Har du et sikringsskap med skrusikringer fra før 1980, bør det byttes. Kost: 22 000 til 40 000 kr i Bergen.` },
    { sporsmal: `Kan jeg installere elbillader i ${bydel.navn}?`, svar: `Ja, men det må gjøres av autorisert elektriker. Elektriker melder arbeidet til BKK (netteier i Bergen). Pris: 10 000 til 30 000 kr.` },
    { sporsmal: `Hva gjør jeg ved strømbrudd i ${bydel.navn}?`, svar: `Sjekk sikringsskapet for utløste sikringer. Vedvarer problemet, kontakt BKK på 55 12 73 53 eller ring døgnvakt elektriker.` },
  ];
    ...
    <>
      ...
      <main id="main-content">
        ...
        <div className="container-site pt-5 pb-2">
          <Breadcrumb items={[{ navn: "Elektriker", href: "/elektriker" }, { navn: "Bergen", href: "/kommune/bergen" }, { navn: `Elektriker ${bydel.navn}` }]} />
        </div>
        ...
        <section className="section-gradient hero-pattern" aria-labelledby="bydel-hero">
          <div className="container-site py-10 sm:py-14"><div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-start">
            <div>
              <div className="badge-primary mb-3"><MapPin className="w-3 h-3" aria-hidden />Bergen – {bydel.navn}</div>
            ...
            <div id="tilbud"><LeadForm kilde={`bydel-bergen-${slug}`} /></div>
          </div></div>
        </section>
        ...
        {bedrifter.length > 0 && (
          <section className="bg-secondary-900 text-white"><div className="container-site py-4">
            <div className="grid grid-cols-3 gap-4 text-center">
              ...
              <div><div className="font-display font-extrabold text-heading-lg text-primary-400">208</div><div className="text-[0.65rem] text-secondary-400 uppercase tracking-wider">Totalt i Bergen</div></div>
        ...
        <section className="section-white py-8 sm:py-12"><div className="container-site max-w-prose">
          ...
          <div className="text-body-md text-secondary-600 leading-relaxed space-y-4">
            ...
            <p>Med {bedrifter.length} registrerte elektrikerbedrifter i {bydel.navn} har du gode muligheter til å finne riktig fagperson. I tillegg tar mange av Bergens øvrige 208 bedrifter oppdrag på tvers av bydelsgrensene.</p>
            <p>De vanligste oppdragene i {bydel.navn} inkluderer oppgradering av sikringsskap, installasjon av elbillader, montering av spotter og feilsøking. Bergens høye nedbørsmengder og fuktighet gjør regelmessig elkontroll ekstra viktig. {bydel.kjennetegn}.</p>
          </div>
        </div></section>
        ...
        <section className="section-subtle py-8 sm:py-10">
          <div className="container-site">
            <h2 className="font-display font-bold text-heading-lg text-secondary-950 mb-2">Elektriker i andre bydeler i Bergen</h2>
            <p className="text-body-sm text-secondary-500 mb-5">Se elektrikere i andre bydeler, eller se hele Bergen samlet.</p>
            <div className="flex flex-wrap gap-2">
              {andreBydeler.map((b) => {
                const bAnt = BERGEN_BEDRIFTER.filter((bed) => inRange(bed.p, b.postRanges)).length;
                return (
              ...
              <Link href="/kommune/bergen" className="badge-neutral hover:bg-primary-50 hover:text-primary-700 hover:border-primary-200 transition-colors">Hele Bergen (208) <ChevronRight className="w-3 h-3" /></Link>
            </div>
          </div>
        </section>
