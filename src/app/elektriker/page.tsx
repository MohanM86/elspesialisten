...
export const metadata: Metadata = {
  ...
  description: "Finn sertifisert elektriker i din norske kommune. Vi dekker alle 357 kommuner i Norge med autoriserte elektrikere. Gratis og uforpliktende tilbud.",
  ...
  return (<><Header /><main id="main-content">
    ...
    <section className="section-gradient hero-pattern" aria-labelledby="oversikt-heading">
      <div className="container-site py-12 text-center">
        <div className="badge-neutral mb-4 mx-auto"><MapPin className="w-3 h-3" aria-hidden />{TOTALT_ELEKTRIKERE.toLocaleString("nb-NO")} registrerte elektrikerbedrifter i Norge</div>
        ...
        return (<div key={fylke.slug} className="mb-12 last:mb-0">
          ...
          {kommunerIFylket.length > 0 ? (
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2.5">
              {kommunerIFylket.map((k) => { const ant = ELEKTRIKER_PER_KOMMUNE[k.slug] || 0; return (<Link key={k.slug} href={`/kommune/${k.slug}`} className="card-flat px-3.5 py-2.5 hover:border-primary-300 hover:bg-primary-50 transition-all group"><div className="text-label font-medium text-secondary-800 group-hover:text-primary-600 transition-colors leading-tight">{k.navn}</div>{ant > 0 && <div className="text-[0.65rem] text-secondary-400 mt-0.5">{ant} bedrifter</div>}</Link>); })}
